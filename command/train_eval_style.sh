...
decoder='linearStyle'  #esrgan||stylenerf||linearStyle
...
CUDA_VISIBLE_DEVICES=1 python eval.py \
  ...
  --chunk 2048 --img_wh 320 240  --nerf_out_dim $nerf_out_dim1 --model_mode $model_mode1 --decoder $decoder --decoder_num_res_blocks $decoder_num_res_blocks --encode_a

CUDA_VISIBLE_DEVICES=1  python eval_metric.py \
  --root_dir $root_dir1 \
  --save_dir $save_dir1 \
  --dataset_name $dataset_name1 --scene_name $exp_name1 \
  --split test_test --img_downscale 2 \
