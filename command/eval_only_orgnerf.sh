...
declare -a StringArray=( "hanerf_encoder_random_down4")
gpu1=7
...
for val in ${StringArray[@]}; do
  ...
  #   --chunk 2048 --img_wh 320 240  --encode_a  #--nerf_out_dim $nerf_out_dim1 --model_mode $model_mode1 --decoder $decoder --decoder_num_res_blocks $decoder_num_res_blocks

  CUDA_VISIBLE_DEVICES=$gpu1  python eval_metric.py \
    --root_dir $root_dir1 \
    --save_dir $save_dir1 \
    --dataset_name $dataset_name1 --scene_name $exp_name1 \
    --split test_test --img_downscale 4 \
    --img_wh 320 240
done